...
import { validateFixtureCorpus } from "./validation.js";
import type { FixtureCorpus } from "./fixture-data.js";
...
if (errors.length > 0) {
  for (const error of errors) console.error(`${error.fixture}${error.path}: [${error.code}] ${error.message}`);
  console.error(`Fixture validation failed with ${errors.length} error(s).`);
  process.exitCode = 1;
} else {
  const fixtureCount = Object.values(corpus.fixtures).reduce((total, group) => total + Object.keys(group).length, 0);
